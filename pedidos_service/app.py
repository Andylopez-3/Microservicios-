...
breaker_state_productos = {
    "estado" : "CERRADO",      #pueder ser CERRADO ,ABIERTO o SEMI_ABIERTO
    "fallos": 0,
    "max_fallos": 3 ,
    "abierto_hasta": None
}

def puede_llamar_productos():
    now = datetime.now(timezone.utc)
    estado = breaker_state_productos["estado"]

    if estado == "CERRADO":
        return True
    if estado == "ABIERTO":
        if now >= breaker_state_productos["abierto_hasta"]:
            breaker_state_productos["estado"] = "SEMI_ABIERTO"
            logging.info("Circuit Breaker de ABIERYO cambiado a SEMI_ABIERTO , probando recuperacion del servicio de productos")
            return True
        return False
    if estado == "SEMI_ABIERTO":
        return True    # en semi abierto 1 intento de prueba 
    return False

def registrar_exito_productos():
    estado_actual = breaker_state_productos["estado"]
    if estado_actual == "SEMI_ABIERTO":
        breaker_state_productos["estado"] = "CERRADO"
        breaker_state_productos["fallos"] = 0
        breaker_state_productos["max_fallos"] = 3
        breaker_state_productos["abierto_hasta"] = None
        logging.info("Circuit Breaker : Productos recuperado , cerrado el circuito")
    elif estado_actual == "CERRADO" and breaker_state_productos["fallos"] > 0:
        breaker_state_productos["fallos"] = 0

def registrar_fallo_productos():
    breaker_state_productos["fallos"] += 1
    estado_actual = breaker_state_productos["estado"]
    logging.error(f"Error en el servicio de productos. Fallos consecutivos: {breaker_state_productos['fallos']}")
    ...
    if estado_actual == "SEMI_ABIERTO":
        ...
        breaker_state_productos["estado"] = "ABIERTO"
        breaker_state_productos["abierto_hasta"] = datetime.now(timezone.utc) + timedelta(seconds=30)
        logging.error("Circuit Breaker de Productos abierto por demasiados fallos por 30 segundos")
...
def manejar_respuesta_producto(respuesta): # funcion para manejar errores del microservicio de productos
    ...
    if status == 200 :  #si todo esta bien no hay error
        registrar_exito_productos()  # registramos el exito en el circuit breaker de productos
        return None
    ...
    if status in (401,403):  # si no esta autorizado para consultar el producto 
    ...
    if status >= 500:     # si hay un error del servidor del microservicio de productos
        registrar_fallo_productos()
        return jsonify({"error": "Error en el servicio de productos"}), 503 # devolvemos error 503 al cliente del servicio de pedidos , por que el servicio de productos no esta disponible
...
def crear_pedido():  # funcion para crear un nuevo pedido
    ...
    if not puede_llamar_productos():
        logging.warning("peticion rechazada : Circuit breaker ABIERTO")
        return jsonify({"error": "Servicio de productos temporalmente no disponible"})  # verificamos el estado del circuit breaker de productos antes de intentar llamar al microservicio de productos
